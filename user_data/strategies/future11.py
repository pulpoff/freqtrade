...
class future11(IStrategy):
    ...
    # Long entry parameters (from pulp2h strategy)
    buy_bb_width_threshold = DecimalParameter(0.13, 0.18, default=0.155, space="buy", optimize=True)
    buy_rsi_lower = IntParameter(28, 38, default=33, space="buy", optimize=True)
    buy_rsi_upper = IntParameter(27, 37, default=32, space="buy", optimize=True)
    buy_macd_hist_threshold = DecimalParameter(-0.04, -0.02, default=-0.028, space="buy", optimize=True)
    buy_volume_multiplier_min = DecimalParameter(2.0, 3.5, default=2.766, space="buy", optimize=True)
    buy_volume_multiplier_max = DecimalParameter(6.0, 9.0, default=7.521, space="buy", optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['avg_price_48h'] = dataframe['close'].rolling(window=576).mean()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if cooldown_ok:
            # Long entries - using indicators from pulp2h strategy
            long_conditions = [
                # Bollinger Band condition
                (dataframe['close'] <= dataframe['bb_lowerband'] * 1.01) &
                (dataframe['close'] > dataframe['bb_lowerband'] * 0.995) &
                ((dataframe['bb_upperband'] - dataframe['bb_lowerband']) / dataframe['bb_middleband'] < self.buy_bb_width_threshold.value),
