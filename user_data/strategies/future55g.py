...
class future55g(IStrategy):
    """
    Enhanced future55c strategy with advanced peak detection for 5-minute timeframes.
    
    Key improvements based on technical analysis research:
    - Replaces binary uptrend blocking with graduated exhaustion scoring (Phase 1)
    - Adds multi-timeframe confirmation (15m/1h context) (Phase 2)
    - Implements divergence-based reversal detection
    - Includes volume confirmation and climax detection
    - Uses composite scoring (6.0+ threshold for high-confidence entries)
    """
    ...
    peak_distance = IntParameter(15, 30, default=20, space="buy", optimize=True)
    peak_prominence_multiplier = DecimalParameter(0.3, 0.7, default=0.5, space="buy", optimize=True)
    
    # Composite scoring thresholds
    reversal_score_threshold = DecimalParameter(5.5, 7.0, default=6.0, space="buy", optimize=True)
    min_volume_ratio = DecimalParameter(1.1, 1.4, default=1.2, space="buy", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """
        Entry logic using composite exhaustion scoring.
        Replaces binary uptrend blocking with graduated scoring system.
        """
        ...
        dataframe.loc[
            volume_ratio > self.volume_ratio_threshold.value,
            'reversal_score'
        ] += 1.0
        ...
        conditions_short = [
            dataframe['reversal_score'] >= self.reversal_score_threshold.value,  # 6.0+ points
            dataframe['rsi'] >= self.rsi_overbought.value,  # Must be overbought
            volume_ratio > self.min_volume_ratio.value,  # Volume confirmation required
            dataframe['close'] > dataframe['ema_fast'],  # Still in uptrend (distribution phase)
            dataframe['volume'] > 0
        ]
        
        # Add multi-timeframe confirmation if enabled
        if self.use_multitimeframe.value == 'True':
            mtf_condition = (dataframe['mtf_15m_confirm'] == 1) | (dataframe['mtf_1h_confirm'] == 1)
            conditions_short.append(mtf_condition)
        
        if conditions_short:
            dataframe.loc[
                reduce(lambda x, y: x & y, conditions_short),
                ['enter_short', 'enter_tag']
            ] = (1, 'exhaustion_short_composite')
        else:
