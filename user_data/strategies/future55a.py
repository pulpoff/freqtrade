...
class future55a(IStrategy):
    ...
    # More aggressive stoploss
    stoploss = -0.30
    
    # More responsive trailing stop
    trailing_stop = True
    ...
    # More aggressive leverage parameters
    ...
    # NEW: 48-hour price position filter - don't short if in bottom 20%
    ...
    # NEW: Additional EMA for trend detection
    ...
    # Higher confluence threshold for better quality shorts
    ...
    startup_candle_count = 600  # Increased from 200 to accommodate 48h lookback
    can_short = True
    can_long = False
    ...
    def calculate_confluence_score(self, dataframe: DataFrame, coin: str) -> DataFrame:
        ...
        signals['rsi_divergence'] = rsi_divergence.astype(int) * 4  # High weight for divergence
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ==========================================
        # NEW: 48-HOUR PRICE POSITION FILTER
        # ==========================================
        # Calculate 48-hour high/low range
        # 48 hours = 576 candles on 5m timeframe
        lookback_48h = 576
        ...
        # Calculate price position within 48h range (0 = bottom, 1 = top)
        # We'll avoid shorting if price_position_48h < threshold (e.g., 0.20)
        dataframe['price_position_48h'] = (
            (dataframe['close'] - dataframe['low_48h']) / 
            (dataframe['high_48h'] - dataframe['low_48h'])
        ).fillna(0.5)  # Default to middle if range is 0
        # ==========================================
        ...
        if self.dp:
            try:
                ...
                informative_1m['deep_oversold'] = (
                    (informative_1m['rsi'] < sell_rsi_lower * 0.8) &  # More aggressive
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ==========================================
        # NEW: 48-HOUR PRICE POSITION FILTER
        # Only short if we're NOT in the bottom 20% of the 48h range
        # ==========================================
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if 'short_exit_signal_1m' in dataframe.columns:
            ...
            exit_signal = (
                (dataframe['rsi'] < sell_rsi_lower * 0.8) &  # More aggressive exit
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float, 
        ...
        if len(dataframe) > 0:
            ...
            # NEW: Final 48h position check
            if 'price_position_48h' in dataframe.columns:
                current_position = dataframe['price_position_48h'].iloc[-1]
                if current_position < self.price_position_48h_threshold.value:
                    return False
        
            if hasattr(self, 'freqai') and self.freqai and 'prediction_probability' in dataframe.columns:
    ...
    def get_trailing_stop_for_coin(self, coin: str) -> dict:
        """Helper method to get trailing stop parameters for a coin"""
