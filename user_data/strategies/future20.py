...
class future20(IStrategy):
    ...
    # Signal combination parameters (which signals to use for entries)
    default_use_price_peak = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    default_use_price_reversal = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    default_use_macd_reversal = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
    
    # Exit signal parameters
    default_use_deep_oversold = CategoricalParameter([True, False], default=True, space="sell", optimize=True)
    default_use_trend_reversal = CategoricalParameter([True, False], default=True, space="sell", optimize=True)
    ...
    def _init_dynamic_coin_params(self):
        ...
        for coin in self._supported_coins:
            ...
            # Signal combination parameters (which signals to use for entries)
            params[f"{coin}_use_price_peak"] = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
            params[f"{coin}_use_price_reversal"] = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
            params[f"{coin}_use_macd_reversal"] = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
            
            # Exit signal parameters
            params[f"{coin}_use_deep_oversold"] = CategoricalParameter([True, False], default=True, space="sell", optimize=True)
            params[f"{coin}_use_trend_reversal"] = CategoricalParameter([True, False], default=True, space="sell", optimize=True)
    ...
    def get_param_value(self, coin: str, param_name: str):
        ...
        default_values = {
            ...
            "use_price_peak": True,
            "use_price_reversal": True,
            "use_macd_reversal": True,
            "use_deep_oversold": True,
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Get signal usage parameters
        use_price_peak = self.get_param_value(coin, 'use_price_peak')
        use_price_reversal = self.get_param_value(coin, 'use_price_reversal')
        use_macd_reversal = self.get_param_value(coin, 'use_macd_reversal')
        use_deep_oversold = self.get_param_value(coin, 'use_deep_oversold')
        use_trend_reversal = self.get_param_value(coin, 'use_trend_reversal')
        ...
        # Combined entry signal with conditional signal inclusion based on hyperopt parameters
        entry_signals = []
        if use_price_peak:
            entry_signals.append('dataframe["price_peak"]')
        if use_price_reversal:
            entry_signals.append('dataframe["price_reversal"]')
        if use_macd_reversal:
            entry_signals.append('dataframe["macd_reversal"]')
        
        # Safety check - if no signals are enabled, use the original combination
        if not entry_signals:
            entry_condition = '(dataframe["price_peak"] | dataframe["price_reversal"] | dataframe["macd_reversal"])'
        else:
            entry_condition = '(' + ' | '.join(entry_signals) + ')'
        
        # Combine with the other conditions
        dataframe['short_entry_signal'] = eval(
            f'{entry_condition} & '
            f'(dataframe["rsi"] > {sell_rsi_upper} * {rsi_upper_scaling}) & '
            f'(dataframe["close"] > dataframe["ema_short"] * (1 + {price_extension_pct}))'
        )
        ...
        # Combined exit signal with conditional signal inclusion based on hyperopt parameters
        exit_signals = []
        if use_deep_oversold:
            exit_signals.append('dataframe["deep_oversold"]')
        if use_trend_reversal:
            exit_signals.append('dataframe["major_trend_reversal"]')
        
        # Safety check - if no signals are enabled, use the original combination
        if not exit_signals:
            exit_condition = '(dataframe["deep_oversold"] | dataframe["major_trend_reversal"])'
        else:
            exit_condition = '(' + ' | '.join(exit_signals) + ')'
        
        # Final exit signal
        dataframe['short_exit_signal'] = eval(exit_condition)
    ...
    def hyperopt_space(self) -> List[Dimension]:
        ...
        space.extend([
            ...
            Categorical([True, False], name='default_use_price_peak'),
            Categorical([True, False], name='default_use_price_reversal'),
            Categorical([True, False], name='default_use_macd_reversal'),
            Categorical([True, False], name='default_use_deep_oversold'),
            Categorical([True, False], name='default_use_trend_reversal')
        ])
        ...
        for coin in self._supported_coins:
            space.extend([
                ...
                # Signal combination parameters
                Categorical([True, False], name=f'{coin}_use_price_peak'),
                Categorical([True, False], name=f'{coin}_use_price_reversal'),
                Categorical([True, False], name=f'{coin}_use_macd_reversal'),
                Categorical([True, False], name=f'{coin}_use_deep_oversold'),
                Categorical([True, False], name=f'{coin}_use_trend_reversal')
            ])
