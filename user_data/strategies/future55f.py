...
class future55f(IStrategy):
    # Reverting to "Complex" ROI - Better for volatility
    minimal_roi = {
        "0": 0.05,
        "10": 0.04,
        "30": 0.02,
    ...
    stoploss = -0.07 # Slightly tightened from -0.30 for safety
    ...
    # 1. Confluence: Kept moderate to ensure we still get trades
    confluence_threshold = IntParameter(5, 9, default=6, space="buy", optimize=True)
    
    # 2. RSI: Slightly relaxed but guarded by new filters
    buy_rsi = IntParameter(30, 45, default=35, space="buy", optimize=True)
    
    # 3. CRITICAL: Tightened ADX (Lower = Safer). 
    # If ADX > 25 and Bearish, we DO NOT buy.
    ...
    def initialize_coin_parameters(self):
        for coin in self._coin_list:
            ...
            setattr(self, f"{coin}_buy_rsi", IntParameter(30, 45, default=35, space="buy", optimize=True))
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Fix Scope Issue (Defined at top level now)
        entry_timeframe = self.get_param_value(coin, 'entry_timeframe')
        ...
        # --- STRICT LONG LOGIC (ANTI-KNIFE) ---
        
        # 1. Downtrend Guard
        # If ADX > 25 (Strong) AND Minus DI > Plus DI (Bear), you are in a waterfall.
        # We strictly block entry here.
        is_strong_downtrend = (
            (dataframe['adx'] > self.adx_threshold.value) & 
            (dataframe['minus_di'] > dataframe['plus_di'])
        )
        
        # 2. Crash Candle Guard
        # Do not buy if the current candle is Red and spread is > 1.5x ATR
        is_crash_candle = (
            (dataframe['close'] < dataframe['open']) & 
            ((dataframe['open'] - dataframe['close']) > (dataframe['atr'] * 1.5))
        )

        # 3. 200 EMA Hard Deck
        # If price is below 1h EMA 200, we are in Macro Bear. 
        # We force RSI to be < 20 (Extreme Dip) to enter.
        if 'ema_200_1h' in dataframe.columns:
            macro_bear_filter = (
                (dataframe['close'] < dataframe['ema_200_1h']) & 
                (dataframe['rsi'] > 20) # If Bearish but RSI not < 20, BLOCK.
            )
        else:
            macro_bear_filter = False
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        coin = self.get_coin_from_pair(pair)
        if not hasattr(self, '_trade_start_reset_times'): self._trade_start_reset_times = {}
        ...
        if trade.is_short:
            try:
                if 'atr' in last_candle:
                    ...
                    if current_rate <= dynamic_exit_price and current_profit > 0.01: return 'atr_based_exit'
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, 
        ...
        stoploss = self.get_trailing_stop_for_coin(coin)['stoploss']
        if stoploss == 0: stoploss = -0.25
