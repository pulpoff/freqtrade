...
class future55d(IStrategy):
    ...
    # New: Downtrend Protection Parameter
    ...
    def initialize_coin_parameters(self):
        for coin in self._coin_list:
            ...
            # Optimized RSI for Longs
            setattr(self, f"{coin}_buy_rsi", IntParameter(20, 45, default=30, space="buy", optimize=True))
            self._coin_parameters[coin]['buy_rsi'] = getattr(self, f"{coin}_buy_rsi")
            
            # Original Short Params
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # --- NEW INDICATORS FOR DOWNTREND PROTECTION ---
        ...
        # --- LONG ENTRY SIGNAL ---
        # Note: Strong downtrend protection is applied in populate_entry_trend
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # --- NEW LONG LOGIC WITH DOWNTREND PROTECTION ---
        
        # 1. Downtrend Protection (Knife Catching Guard)
        # If ADX is high (> 25) AND Minus DI > Plus DI, it is a strong downtrend. Do not buy.
        is_strong_downtrend = (dataframe['adx'] > self.adx_threshold.value) & (dataframe['minus_di'] > dataframe['plus_di'])
        
        # 2. 48H High Protection (Don't buy the top)
        not_at_top = dataframe['price_position_48h'] < 0.80
        ...
        dataframe.loc[
            ...
            (~is_strong_downtrend) &  # CRITICAL FIX
            (not_at_top),
            'enter_long'
        ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if 'long_exit_signal_1m' in dataframe.columns:
            ...
            # Simple backup exit if 1m not available
            dataframe.loc[
                (dataframe['rsi'] > 75) & 
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float, 
        ...
        if len(dataframe) > 0:
            ...
            # --- Long Validations ---
            if side == 'long':
                 # Macro Trend Check (Optional safety)
                 # If price is below 1h EMA 200 (Macro Bear), be extra careful
                 if 'ema_200_1h' in dataframe.columns:
                     last_candle = dataframe.iloc[-1]
                     if last_candle['close'] < last_candle['ema_200_1h']:
                         # We are in macro bear, only accept deep dips
                         if last_candle['rsi'] > 30: # Reject if RSI not very low
                             return False
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        if trade.is_short:
            # ... Existing Short Custom Exit Logic ...
            try:
        ...
        else: # --- LONG CUSTOM EXIT ---
            try:
                # ROI Logic for Longs (Reusing same thresholds for simplicity or mirroring)
                roi_thresholds = {
                    'high_vol': {"0": 0.04, "1": 0.03, "3": 0.02, "10": 0.01, "30": 0},
                    'medium_vol': {"0": 0.035, "2": 0.025, "5": 0.015, "20": 0.005, "45": 0},
                    'low_vol': {"0": 0.03, "3": 0.02, "10": 0.01, "30": 0.003, "90": 0}
                ...
                # Fast exit if RSI becomes overbought
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, 
        ...
        # Dynamic stoploss logic (tighten as profit grows)
    ...
    def get_strategy_parameters(self) -> Dict[str, Any]:
        ...
        # (Parameters dictionary construction same as original, omitted for brevity in this specific method block)
        return params
