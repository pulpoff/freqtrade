...
class future66a(IStrategy):
    ...
    def feature_engineering_expand_basic(self, dataframe: DataFrame, metadata: dict, **kwargs) -> DataFrame:
        ...
        # 24h price position - critical for ML to learn
        candles_24h = 288
    ...
    def populate_core_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ============================================
        # 24H PRICE RANGE POSITION (Key Filter)
        # ============================================
        # 288 candles = 24 hours on 5m timeframe
        candles_24h = 288
        ...
        # Filter zones
        dataframe['near_24h_high'] = dataframe['position_24h'] > 85  # Top 15%
        dataframe['near_24h_low'] = dataframe['position_24h'] < 15   # Bottom 15%
        
        # Also check recent momentum to avoid chasing
        # Don't long after big pump, don't short after big dump
        dataframe['pump_12h'] = (
            (dataframe['close'] - dataframe['close'].shift(144)) / 
            dataframe['close'].shift(144) * 100
        )
        dataframe['recent_pump'] = dataframe['pump_12h'] > 8  # Up more than 8% in 12h
        dataframe['recent_dump'] = dataframe['pump_12h'] < -8  # Down more than 8% in 12h
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # CRITICAL FILTER: Don't short at 24h lows (no room to fall)
        short_position_ok = ~dataframe['near_24h_low']
        
        # Don't short after a massive dump (likely bounce coming)
        short_momentum_ok = ~dataframe['recent_dump']
        
        # Combine: Need swing/pattern AND rsi confirmation AND position filter
        short_signal = (
        ...
        # CRITICAL FILTER: Don't long at 24h highs (no room to rise)
        long_position_ok = ~dataframe['near_24h_high']
        
        # Don't long after a massive pump (likely pullback coming)
        long_momentum_ok = ~dataframe['recent_pump']
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
        ...
        # 24H POSITION CHECK (extra safety)
        position_24h = last_candle.get('position_24h', 50)
        if side == 'long' and position_24h > 80:  # Don't long in top 20%
            return False
        if side == 'short' and position_24h < 20:  # Don't short in bottom 20%
            return False
